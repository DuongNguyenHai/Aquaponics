// Seed-TCP/server-select.cc

// Nguyen Hai Duong
// Date : 11 jun 2016

// compile : g++ server-select.cc ../lib/TCP-IP.cc ../lib/seed-log.cc -o server-select


#include "../lib/TCP-IP.h"

inline void HandleClient(int clntSock, fd_set *set){
        ...
        for (soc = 0; soc <= maxsock; soc++) {
            if (FD_ISSET(soc, &readsocks)) {
                ...
                if (soc == servSock) {
                    ...
                    /* HandleClient read or disconnection */
